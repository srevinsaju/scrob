...
use log::{warn, debug};
use types::song::Song;
use config::{APP_NAME, APP_VERSION, APP_URL};
...
pub fn search_musicbrainz(res: &Song) -> Song {
    ...
    .build().expect("Couldn't instantiate musicbrainz client");


    let search_string = format!("{} {}", res.track, res.artist);
    let resp = client.get(format!("http://musicbrainz.org/ws/2/release/?query={}&fmt=json", search_string)).send();

    if let Err(e) = resp {
        warn!("Error when trying to fetch the current song from musicbrainz: {}", e);
        return res.to_owned();
    }
    ...
    if let Err(e) = resp {
        warn!("Error when trying to parse the current song from musicbrainz: {}", e);
        return res.to_owned();
    }
    let resp = resp.unwrap();
    ...
    debug!("{:?}", first_release);
    let artist_credit = first_release.get("artist-credit").unwrap().as_array().unwrap();
    let release_group = first_release.get("release-group").unwrap().as_object().unwrap();
    ...
    debug!("Musicbrainz search result: {} - {} - {}", title, artist, album);
